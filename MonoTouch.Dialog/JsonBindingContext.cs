			...
			new Dictionary<string, Func<JsonObject, JsonDialogViewController, JsonValue, Element>>(){
				{"EntryElement", (json, dvc, data)=>{
					return new EntryElement(json.s("caption"), json.s("placeholder"), data==null? json.s("value") : data.CleanString(), json.b("ispassword")){
							...
							AutoCapitalize = (UITextAutocapitalizationType)Enum.Parse(typeof(UITextAutocapitalizationType), json.s("capitalization") ?? "Sentences")
						}; }},
				{"MultilineEntryElement", (json, dvc, data)=>{
					return new MultilineEntryElement(json.s("caption"), data==null? json.s("value") : data.CleanString()){
							KeyboardType = (UIKeyboardType)Enum.Parse(typeof(UIKeyboardType), json.s("keyboard") ?? "Default")
						}; }},
				...
				{"StringElement", (json, dvc, data)=>{
					...
					return new StringElement(json.s("caption"), v, json.a(dvc));
				}},
				...
				{"RadioElement", (json, dvc, data)=>{
					...
					return new RootElement(json.s("caption"), new RadioGroup(null, selected)) {
						new Section(){
							radios.ToArray()		
						}
					};
				...
				{"SimpleImageElement", (json, dvc, data)=>{ return  new SimpleImageElement(UIImage.FromBundle(json.s("url")));}},
